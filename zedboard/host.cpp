...
int main(int argc, char** argv)
{
  ...
  if ( myfile.is_open() ) {
    ...
    //Send all test inputs to device
    for (int i = 0; i < N; ++i ) {
      // Read input from array and split into two 32-bit words
      digit test_digit = inputs[i];
      bit64_t digit_i;
      digit_i(test_digit.length()-1,0) = test_digit(test_digit.length()-1,0);
    ...
    for (int i = 0; i < N; ++i ) {
      ...
      //Assign to interpreted digit
      interpreted_digit = d_out;
